...
//  calling glBindBuffer more than necessary in the render loop. 2019-03-15: OpenGL: Added a GL call + comments in
...
//  for any version over 410 (e.g. 430, 450). 2018-11-30: Misc: Setting up io.BackendRendererName so it can be displayed
//  in the About Window. 2018-11-13: OpenGL: Support for GL 4.5's glClipControl(GL_UPPER_LEFT) / GL_CLIP_ORIGIN.
//  2018-08-29: OpenGL: Added support for more OpenGL loaders: glew and glad, with comments indicative that any loader
//  can be used. 2018-08-09: OpenGL: Default to OpenGL ES 3 on iOS and Android. GLSL version default to "#version 300
//  ES". 2018-07-30: OpenGL: Support for GLSL 300 ES and 410 core. Fixes for Emscripten compilation. 2018-07-10: OpenGL:
//  Support for more GLSL versions (based on the GLSL version string). Added error output when shaders fail to
//  compile/link. 2018-06-08: Misc: Extracted imgui_impl_opengl3.cpp/.h away from the old combined GLFW/SDL+OpenGL3
//  examples. 2018-06-08: OpenGL: Use draw_data->DisplayPos and draw_data->DisplaySize to setup projection matrix and
//  clipping rectangle. 2018-05-25: OpenGL: Removed unnecessary backup/restore of GL_ELEMENT_ARRAY_BUFFER_BINDING since
//  this is part of the VAO state. 2018-05-14: OpenGL: Making the call to glBindSampler() optional so 3.2 context won't
//  fail if the function is a nullptr pointer. 2018-03-06: OpenGL: Added const char* glsl_version parameter to
//  ImGui_ImplOpenGL3_Init() so user can override the GLSL version e.g. "#version 150". 2018-02-23: OpenGL: Create the
//  VAO in the render function so the setup can more easily be used with multiple shared GL context. 2018-02-16: Misc:
//  Obsoleted the io.RenderDrawListsFn callback and exposed ImGui_ImplSdlGL3_RenderDrawData() in the .h file so you can
//  call it yourself. 2018-01-07: OpenGL: Changed GLSL shader version from 330 to 150. 2017-09-01: OpenGL: Save and
//  restore current bound sampler. Save and restore current polygon mode. 2017-05-01: OpenGL: Fixed save and restore of
//  current blend func state. 2017-05-01: OpenGL: Fixed save and restore of current GL_ACTIVE_TEXTURE. 2016-09-05:
//  OpenGL: Fixed save and restore of current scissor rectangle. 2016-07-29: OpenGL: Explicitly setting
...
// glLoadGen, etc.). If you happen to be developing a new feature for this backend (imgui_impl_opengl3.cpp):
...
#ifdef IMGUI_IMPL_OPENGL_MAY_HAVE_POLYGON_MODE
    // Desktop OpenGL 3.0 and OpenGL 3.1 had separate polygon draw modes for front-facing and back-facing faces of
    // polygons
    if ( bd->HasPolygonMode ) {
...
bool ImGui_ImplOpenGL3_CreateFontsTexture() {
    ...
    io.Fonts->GetTexDataAsRGBA32(
            &pixels, &width,
            &height ); // Load as RGBA 32-bit (75% of the memory is wasted, but default font is so small) because it is
